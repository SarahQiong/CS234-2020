...
def policy_evaluation(P, nS, nA, policy, gamma=0.9, tol=1e-3):
    ...
    while True:
        ...
        for s in range(nS):
            ...
            # print(s, np.abs(val- value_function[s]))
    ...
    for s in range(nS):
        ...
        for a in range(nA):
            for prob, next_state, reward, terminal in P[s][a]:
                q_vals[a] += prob * (reward + gamma * value_from_policy[next_state])
...
def value_iteration(P, nS, nA, gamma=0.9, tol=1e-3):
    ...
    while True:
        ...
        for s in range(nS):
            ...
            for a in range(nA):
                for prob, next_state, reward, terminal in P[s][a]:
                    vals[a] += prob * (reward + gamma * value_function[next_state])
            best = np.max(vals)
            delta = max(delta, np.abs(val- best))
