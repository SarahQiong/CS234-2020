...
class Linear(DQN):
    ...
    def add_placeholders_op(self):
        ...
        img_height, img_width, nchannels = state_shape[0], state_shape[
            1], state_shape[2]
    ...
    def get_q_values_op(self, state, scope, reuse=False):
        ...
        out = layers.fully_connected(state_flatten,
                                     num_actions,
                                     reuse=reuse,
                                     scope=scope,
                                     activation_fn=None)
    ...
    def add_update_target_op(self, q_scope, target_q_scope):
        ...
        q_collect = tf.get_collection(tf.compat.v1.GraphKeys.GLOBAL_VARIABLES,
                                                scope=q_scope)
        target_q_collect = tf.get_collection(
    ...
    def add_loss_op(self, q, target_q):
        ...
        Q_sample = self.r + mask * self.config.gamma * tf.reduce_max(target_q,
                                                                     axis=1)
    ...
    def add_optimizer_op(self, scope):
        ...
        scope_vars = tf.compat.v1.get_collection(tf.compat.v1.GraphKeys.GLOBAL_VARIABLES,
                                                 scope=scope)
        grads_and_vars = optimizer.compute_gradients(self.loss, scope_vars)
        if self.config.grad_clip:
            grads_and_vars = [(tf.clip_by_norm(item[0],
                                               self.config.clip_val), item[1])
